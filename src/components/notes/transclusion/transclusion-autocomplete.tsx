...
import type { TransclusionSearchResult } from '@/lib/notes/types';
import { FileTextIcon, Loader2Icon } from 'lucide-react';

// ═══════════════════════════════════════════════════════════════════
// TransclusionAutocomplete — Fuzzy search for blocks when typing ((
// Shows matching blocks with preview text
// ═══════════════════════════════════════════════════════════════════

interface TransclusionAutocompleteProps {
  query: string;
  position: { top: number; left: number };
  onSelect: (result: TransclusionSearchResult) => void;
  onClose: () => void;
  selectedIndex?: number;
}

export function TransclusionAutocomplete({
  query,
  position,
  onSelect,
  onClose,
  selectedIndex = 0,
}: TransclusionAutocompleteProps) {
  const { resolvedTheme } = useTheme();
  const isDark = resolvedTheme === 'dark' || resolvedTheme === 'oled' || resolvedTheme === 'cosmic';

  const [results, setResults] = useState<TransclusionSearchResult[]>([]);
  ...
  useEffect(() => {
    const searchBlocks = async () => {
      ...
      try {
        setLoading(true);
        const searchResults = await invoke<TransclusionSearchResult[]>('search_blocks_for_transclusion', {
          query: query.trim(),
          limit: 10,
        });
  ...
  // Don't render if no query and no results
        ...
        results.map((result, idx) => {
          ...
          return (
            ...
            >
              ...
              >
                <FileTextIcon style={{ width: '13px', height: '13px', color: 'var(--pfc-accent)' }} />
              </div>
              <div style={{ minWidth: 0, flex: 1 }}>
                ...
                >
                  {result.block_content || '(empty block)'}
                </div>
              </div>
            </button>
          );
